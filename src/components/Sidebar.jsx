...
import { Home, PieChart, List, PlusCircle, Settings, CreditCard } from 'lucide-react';
import clsx from 'clsx';

const Sidebar = ({ activeTab, setActiveTab, onBackToHub }) => {
  const menuItems = [
    ...
    <aside className="glass" style={{ width: '280px', display: 'flex', flexDirection: 'column', borderRight: '1px solid var(--border-light)', padding: '2rem' }}>
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          <div style={{
            width: '40px', height: '40px', borderRadius: '12px', background: 'linear-gradient(135deg, var(--accent-primary), var(--accent-secondary))',
            display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', boxShadow: 'var(--shadow-glow)'
          }}>
            <CreditCard size={24} />
          </div>
          <h1 className="text-gradient" style={{ fontSize: '1.5rem', fontWeight: 700, margin: 0 }}>Orbit</h1>
        </div>
        <button 
          className="btn-icon" 
          onClick={onBackToHub} 
        ...
        {menuItems.map(item => (
          <button
            ...
            style={{
              ...
              borderRadius: 'var(--radius-md)', background: activeTab === item.id ? 'rgba(99, 102, 241, 0.15)' : 'transparent',
          ...
          >
            <item.icon size={20} style={{ color: activeTab === item.id ? 'var(--accent-primary)' : 'inherit' }} />
            {item.label}
          </button>
        ))}
      </nav>
      ...
      <div style={{ marginTop: 'auto', paddingTop: '2rem', borderTop: '1px solid var(--border-light)' }}>
        <button className="btn btn-primary w-full justify-center" onClick={() => document.getElementById('add-modal')?.showModal()}>
          <PlusCircle size={20} />
          New Transaction
        </button>
      </div>
